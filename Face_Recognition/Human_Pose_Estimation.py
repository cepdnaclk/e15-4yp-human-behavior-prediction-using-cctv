...
def detectPose():
    ...
    video = cv2.VideoCapture(0) #'../Assets/Videos/common2.mp4')

    while True:
        t = time.time()
        check, frame = video.read()
        if check:
            frame = cv2.resize(frame, (0, 0), cv2.INTER_AREA, 1, 1)
            ...
            cv2.putText(frame, f'FPS:{fps}', (6, 18), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
            cv2.imshow('Human Skeleton', frame)
            key = cv2.waitKey(1)
            if key == 27:
                break
        else:
